...
import { Search, Coins, User, Package, Sparkles, Store, Shield, Orbit, Layers } from 'lucide-react'
...
export default function MarketplacePage() {
  ...
  const filteredListings = listings.filter(listing => 
    listing.frame.name.toLowerCase().includes(searchTerm.toLowerCase())
  )
      ...
      {filteredListings.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/20 border border-slate-800/50 rounded-2xl border-dashed">
          ...
          <p className="text-sm text-slate-500 max-w-md">
            Não há molduras disponíveis no mercado para os filtros selecionados no momento. Tente buscar por algo diferente!
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 min-[480px]:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-4 sm:gap-6">
          {filteredListings.map((listing) => {
            ...
            return (
              <Link
                key={listing.id}
                href={`/worldo/cosmetics/${listing.frameId}`}
