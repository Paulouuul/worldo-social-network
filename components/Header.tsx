                ...
                <>
                  ...
                  >
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-3 h-3 opacity-50 transition-transform duration-300 ${isCosmeticsOpen ? 'rotate-180' : ''}`}
                      />
                      <span
                        className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-linear-to-r from-blue-500 to-purple-500 transition-all duration-300 rounded-full ${isCosmeticsOpen ? 'w-8/12' : 'w-0'}`}
                      ></span>
                    </button>

                    {/* Menu Flutuante */}
                    <div
                      className={`absolute top-full right-0 mt-2 w-52 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] p-2 transition-all duration-300 z-50 transform origin-top-right ${isCosmeticsOpen ? 'opacity-100 visible scale-100' : 'opacity-0 invisible scale-95'}`}
                    >
                      <Link
      ...
      >
        <div className="flex flex-col h-full pt-6 pb-8 relative overflow-y-auto custom-scrollbar">
          ...
          <div className="px-5 sm:px-6 pb-4 flex items-center gap-3 border-b border-white/5 shrink-0">
            ...
            <span className="text-xl font-black tracking-wider galaxy-text select-none">Menu</span>
          </div>
