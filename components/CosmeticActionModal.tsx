...
import { 
  Tag, Store, Edit2, MinusCircle, X, AlertTriangle, 
  Loader2, Coins, Box, ExternalLink, ArrowLeft 
} from 'lucide-react'
...
// ==========================================
export function CosmeticActionModal({ item, mode, onClose, onSuccess, avatarUrl }: CosmeticActionModalProps) {
      ...
      <div className={`bg-slate-900 border ${rarityConfig.borderClass} rounded-2xl max-w-md w-full shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-200 flex flex-col max-h-[95vh] sm:max-h-[90vh]`}>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-800/60 flex items-center justify-between bg-slate-950/40 shrink-0">
          <div className="flex items-center gap-2">
            {currentMode === 'sell' && <><Tag className="w-4 h-4 text-purple-400 shrink-0" /><h2 className="text-xs sm:text-sm font-black text-slate-200 uppercase tracking-wider truncate">Criar Ordem de Venda</h2></>}
            {currentMode === 'view' && <><Store className="w-4 h-4 text-emerald-400 shrink-0" /><h2 className="text-xs sm:text-sm font-black text-slate-200 uppercase tracking-wider truncate">Detalhes do Anúncio</h2></>}
            {currentMode === 'edit' && <><Edit2 className="w-4 h-4 text-indigo-400 shrink-0" /><h2 className="text-xs sm:text-sm font-black text-slate-200 uppercase tracking-wider truncate">Editar Preço</h2></>}
            {currentMode === 'remove' && <><MinusCircle className="w-4 h-4 text-rose-400 shrink-0" /><h2 className="text-xs sm:text-sm font-black text-slate-200 uppercase tracking-wider truncate">Retirar do Mercado</h2></>}
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300 transition p-1 shrink-0" disabled={submitting}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5">
          ...
          <div className="text-center mb-4">
             <h3 className={`font-bold text-lg ${rarityConfig.textClass}`}>{item.frame.name}</h3><br/>
             <p className={`text-xs font-semibold tracking-wider uppercase ${rarityConfig.textClass}`}>------------------------</p>
              <p className={`text-xs font-semibold tracking-wider uppercase ${rarityConfig.textClass}`}>{item.frame.rarity}</p>
              <p className={`text-xs font-semibold tracking-wider uppercase ${rarityConfig.textClass}`}>------------------------</p>
          </div>
            ...
            <div className="space-y-4 sm:space-y-5 animate-in slide-in-from-right-4 duration-200">
              <div className="bg-slate-950/50 rounded-xl p-3 sm:p-4 border border-slate-800/50 flex flex-col gap-2 sm:gap-3 shadow-inner">
                <div className="flex justify-between items-center border-b border-slate-800/50 pb-2">
                  ...
                  <span className="text-sm font-black text-amber-500 flex items-center gap-1.5"><Coins className="w-3.5 h-3.5 sm:w-4 sm:h-4"/> {item.resalePrice}</span>
                </div>
          ...
          {currentMode === 'sell' && (
            <form onSubmit={handleSell} className="space-y-4 animate-in slide-in-from-right-4 duration-200">
              ...
              <div className="bg-purple-500/5 border border-purple-500/10 rounded-xl p-3 sm:p-3.5 flex justify-between items-center text-xs">
                <span className="text-slate-400 font-medium text-[10px] sm:text-xs">Rendimento Total:</span>
          ...
          {currentMode === 'edit' && (
            <form onSubmit={handleUpdatePrice} className="space-y-4 animate-in slide-in-from-left-4 duration-200">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-slate-800/60">
                <button type="submit" disabled={submitting || price === item.resalePrice} className="flex-[1.5] bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm py-3 px-4 rounded-xl transition-[transform,border-color,background-color,box-shadow] shadow-lg shadow-indigo-900/20 disabled:opacity-50 flex items-center justify-center gap-2">
                  {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Salvar Alteração'}
                </button>
                <button type="button" onClick={() => setCurrentMode('view')} disabled={submitting} className="flex-1 bg-slate-800/50 border border-slate-700/50 hover:bg-slate-800 text-slate-300 font-semibold text-sm py-3 px-4 rounded-xl transition flex items-center justify-center gap-2">
          ...
          {currentMode === 'remove' && (
            <form onSubmit={handleRemoveItems} className="space-y-4 animate-in slide-in-from-right-4 duration-200">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-slate-800/60">
                <button type="submit" disabled={submitting} className="flex-[1.5] bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm py-3 px-4 rounded-xl transition-[transform,border-color,background-color,box-shadow] shadow-lg shadow-rose-900/20 flex items-center justify-center gap-2">
